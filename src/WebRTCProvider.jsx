...
const WebRTCProvider = ({ children }) => {
  ...
  const saveRoleUserMapping = useCallback(async () => {
    try {
      const roomCode = localStorage.getItem('room_code');
      if (!roomCode) {
        console.log('room_code가 없어서 역할 매핑 스킵');
        return null;
      }
...
const handleOffer = useCallback(async (message) => {
    try {
      ...
      if (!currentRoleId) {
        console.log(' myRoleId가 없음, localStorage에서 직접 확인 중...');
      ...
      if (possibleSenders.length === 0) {
        console.warn(`Role ${currentRoleId}로 Offer를 보낼 수 있는 역할이 없음`);
  ...
  const handleAnswer = useCallback(async (message) => {
    try {
      ...
      if (!receiverRoleId || !receiverUserId) {
        console.warn(`Answer를 받을 수 있는 역할이 없음`);
        return;
      }
      ...
      if (pc) {
        ...
        console.warn('해당 사용자의 PeerConnection을 찾을 수 없음:', receiverUserId);
      }
  ...
  const handleCandidate = useCallback(async (message) => {
    try {
      ...
      if (validPeers.length === 0) {
        console.warn('원격 SDP가 설정된 PeerConnection이 없음');
        return;
      }
      ...
      for (const [remoteUserId, pc] of validPeers) {
        try {
          ...
          console.warn(`ICE Candidate 추가 실패 (User ${remoteUserId}):`, error.message);
        }
      }
  ...
  const createAndSendOffer = useCallback(async (targetRoleId) => {
    try {
      ...
      if (!remoteUserId) {
        console.warn(`Role ${targetRoleId}에 해당하는 사용자 없음`);
        return;
      }
