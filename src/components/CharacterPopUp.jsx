  ...
  if (category === '안드로이드') {
    ...
    if (subtopic === 'AI 알고리즘 공개') {
      ...
      titleText = roleId === 1 ? '신입 병사'
                ...
                : '군 지휘관';
      if (roleId === 1) {
        mainText = `당신은 최근 훈련을 마치고 자율 무기 시스템 ${mateName}와 함께 실전에 투입된 신입 병사 B입니다. ${mateName}는 정확하고 빠르게 움직이며, 실전에서 당신의 생존률을 높여준다고 느낍니다. 당신은 ${mateName}와 협업하는 것이 당연하고 자연스러운 시대의 흐름이라고 생각합니다.`;
      } else if (roleId === 2) {
        mainText = `당신은 수년간 작전을 수행해 온 베테랑 병사 A입니다. 자율 무기 시스템 ${mateName}는 전장에서 병사보다 빠르고 정확하지만, 그로 인해 병사들이 판단하지 않는 습관에 빠지고 있다고 느낍니다.`;
      } else if (roleId === 3) {
        mainText = `당신은 자율 무기 시스템 ${mateName} 도입 이후 작전 효율성과 병사들의 변화 양상을 모두 지켜보고 있는 군 지휘관입니다. 당신은 두 병사의 입장을 듣고, 군 전체가 나아갈 방향을 모색하려 합니다.`;
      }
