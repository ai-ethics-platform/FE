...
export default function CD3() {
  ...
  // 이미지/텍스트 매핑 로직 (Player 3 전용)
  const getAsset = () => {
    const isEn = lang !== 'ko';
    if (!isAndroid) { // AWS 분기
      const awsMap = {
        'AI 알고리즘 공개': { ko: AWS_1, en: AWS_1_en, t: t.cd3_aws_1 },
        'AWS의 권한': { ko: AWS_2, en: AWS_2_en, t: t.cd3_aws_2 },
        '사람이 죽지 않는 전쟁': { ko: AWS_3, en: AWS_3_en, t: t.cd3_aws_3 },
        'AI의 권리와 책임': { ko: AWS_4, en: AWS_4_en, t: t.cd3_aws_4 },
        'AWS 규제': { ko: AWS_5, en: AWS_5_en, t: t.cd3_aws_5 },
      };
      ...
      if (subtopic === t_map.andOption2_1 || subtopic === t_map.andOption2_2) 
        return { img: isEn ? player3DescImg_title2_en : player3DescImg_title2, txt: t.cd3_android_council };
      if (subtopic === t_map.andOption3_1) 
        return { img: isEn ? player3DescImg_title3_en : player3DescImg_title3, txt: t.cd3_android_international };
      return { img: isEn ? player3DescImg_title1_en : player3DescImg_title1, txt: t.cd3_android_home };
    }
  };
  ...
  return (
    <Layout round={round} subtopic={subtopic} me="3P" onBackClick={() => navigate('/game01')}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 32, marginTop: 22 }}>
        <img src={descImg} alt="Player 3" style={{ width: 264, height: 336, objectFit: 'contain' }} onError={(e) => e.currentTarget.src = defaultimg} />
