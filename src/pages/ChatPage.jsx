...
import "../components/chat.css"; // ↓ 2) CSS 포함
...
export default function ChatPage() {
  ...
  const [loading, setLoading] = useState(false); // ← 응답 대기 중
  ...
  // 응답 표준화(백엔드 키가 달라도 흡수)
  function normalize(res) {
    const text = res?.output ?? res?.text ?? res?.message ?? "";
    const nextStep = res?.next?.step;
    const newContext = res?.context;
    return { text, nextStep, newContext };
  }
  ...
  async function handleSend(userText) {
    ...
    try {
      ...
      const payload = { step, input: isInit ? "" : raw, context, prompt, init: isInit };
      ...
      if (nextStep && ORDER.includes(nextStep)) setStep(nextStep);
    } catch (e) {
