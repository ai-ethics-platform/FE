...
export default function SelectHomeMate() {
  ...
  // 🔥 커스텀 훅들 사용 (GameIntro와 동일)
  const { isHost, sendNextPage } = useHostActions();
  
  // 🔥 페이지 이동 메시지 핸들러 (useWebSocketNavigation 사용)
  useWebSocketNavigation(navigate, {
    nextPagePath: '/matename'  // 다음 페이지 경로
  });
  ...
  const handleCharacterSelect = (idx) => {
    if (!isHost) {
      console.log('⚠️ [SelectHomeMate] 방장이 아니므로 캐릭터 선택 불가');
  ...
  const handleContinue = async () => {
    console.log('➡️ [SelectHomeMate] 다음 버튼 클릭');
    ...
    try {
      ...
      // 2. AI 선택 성공 후 next_page 브로드캐스트 전송
      console.log('👑 [SelectHomeMate] 방장이므로 next_page 브로드캐스트 전송');
      
      const success = sendNextPage();
      if (success) {
        console.log('📤 [SelectHomeMate] next_page 브로드캐스트 전송 성공');
        console.log('📡 [SelectHomeMate] 서버가 모든 클라이언트에게 브로드캐스트 중...');
        console.log('🎯 [SelectHomeMate] useWebSocketNavigation이 브로드캐스트를 받아서 자동으로 페이지 이동 처리');
      } else {
        console.error('❌ [SelectHomeMate] next_page 브로드캐스트 전송 실패');
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            {images.map((src, idx) => (
              <img
                ...
                src={src}
                alt={`Character ${idx + 1}`}
                onClick={() => handleCharacterSelect(idx)} // 🔥 방장 전용 핸들러
                style={{
                  width: 264,
                  height: 360,
                  objectFit: 'cover',
                  borderRadius: 4,
                  cursor: isHost ? 'pointer' : 'not-allowed', // 🔥 방장이 아니면 커서 변경
                  border: activeIndex === idx ? `2px solid #354750` : 'none',
                  transform: activeIndex === idx ? 'scale(1.01)' : 'scale(1)',
                  transition: 'all 0.2s ease-in-out',
                  opacity: isHost ? 1 : 0.7, // 🔥 방장이 아니면 반투명
                  filter: isHost ? 'none' : 'grayscale(30%)', // 🔥 방장이 아니면 흐리게
                }}
              />
            ))}
          </div>
